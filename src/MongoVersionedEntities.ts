...
import { Cursor, EntityOwner, EntityPatch, Injector, PartialRecord, VersionedEntity, VersionedEntityStore } from "@ninjs/vef";
import { MongoServerEntityStore } from "./MongoServerEntities";
import { useMongo } from "./utils/Mongo";

export class MongoVersionedEntityStore<TEntity extends VersionedEntity> implements VersionedEntityStore<TEntity> {
  ...
  async useCollection() {
    ...
    const collection = mongo.collection<VersionedEntity>(this.options.name);
  ...
  async versionOf(id: string) {

    const collection = await this.useCollection();

    const one = await collection.findOne<Pick<VersionedEntity, "version">>({
      _id: {
        $eq: id
      }
    }, {
  ...
  async set(id: string, version: number, entity: TEntity, filter: Filter<TEntity> = {}) {
  ...
  async patch(id: string, version: number, patch: Partial<TEntity>, filter: Filter<TEntity> = {}) {
